...
`runScrape` calls `store.EnsureMenuSchema(ctx)` at startup — same pattern as `runIndex` calls `EnsureSchema` (note: Postgres `EnsureSchema` is now a no-op; Postgres tables are created by `golang-migrate` via `go run . db migrate-up`. `EnsureMenuSchema` still creates Weaviate/Pinecone collections). `scrapeCmd` is registered via `rootCmd.AddCommand(scrapeCmd)` in `cli/scrape.go`'s `init()`.
