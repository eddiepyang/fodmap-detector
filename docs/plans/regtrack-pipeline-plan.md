...
| Cron scheduling | **river `PeriodicJob`** — register at client construction time. **Important caveat**: schedules live *in-memory on the leader*, so a leadership-change gap can skip firings ([river docs](https://riverqueue.com/docs/periodic-jobs) say plainly: "the new leader will evaluate all the schedules it knows about starting at the current time"). We mitigate with `RunOnStart: true` plus job-level uniqueness (e.g. unique-per-day for daily schedules) so a missed firing fires once on the next leader's startup without duplicating. Guaranteed no-miss requires River Pro's durable periodic jobs — out of scope. |
...
- **`regtrack/store/`** — Postgres DDL for `sources`, `extraction_rules`, `regulatory_updates`. Raw SQL via `pgx`, matching [auth/postgres_store.go](../../auth/postgres_store.go). River's own tables come from `river migrate-up` and are not in our migrations.
...
- Domain migrations for `sources`, `extraction_rules`, `regulatory_updates` run via pgx (not `database/sql`), invoked from `cli/regtrack_migrate.go` alongside `river migrate-up`. We do **not** piggyback on `auth.PostgresStore`'s constructor-time `CREATE TABLE IF NOT EXISTS` pattern — regtrack migrations are explicit, run by an operator, so we can evolve the schema without touching auth.
...
2. **Schedule fires**: river `PeriodicJob` enqueues a `ScrapeJobArgs{SourceID, URL}` row. (Missed firings from leader-election gaps are *not* automatic — the `RunOnStart: true` + unique-per-day key trick from step 1 catches them on next startup. See Risks.)
...
- **New**: `regtrack/store/schema.sql` (only `sources`, `extraction_rules`, `regulatory_updates` — river's DDL comes from `river migrate-up`)
...
- **Extend**: [cli/root.go](../../cli/root.go) — new subcommand `regtrack add-source <url> --cron …` (writes to `sources` and triggers a river `PeriodicJob` re-registration on next boot, or via signal)
- **New**: `cli/regtrack_migrate.go` — wraps `river migrate-up` for setup, mirrors existing migration ergonomics
...
- **Extend**: [start.sh](../../start.sh) — bring up Postgres alongside Weaviate, then call `go run . regtrack migrate-up` once before `serve` (creates both river's tables via `river migrate-up` *and* our domain tables).
...
- **Periodic-job missed firings on leader gaps.** River's `PeriodicJob` schedules are in-memory on the leader. A leader-election gap can skip firings (their docs example: a midnight job missing when failover happens between 11:59:59.99 and 12:00:00.05). Mitigation in this plan: `RunOnStart: true` + unique-per-day job keys so the next leader's startup fires once if the prior tick was missed. Guaranteed no-miss requires River Pro durable periodic jobs.
...
- **Integration**: spin up Postgres + Weaviate via existing [docker-compose.yaml](../../docker-compose.yaml), run `river migrate-up`, register a fixture source pointing at an `httptest.NewServer`, boot the daemon, wait for one `PeriodicJob` tick, assert: row in `regulatory_updates`, vector in Weaviate, file in `data/bronze/`, `extraction_rules` row with `status='proposed'`. Tick again, assert `RulePromotionWorker` runs and the rule becomes `status='active'`.
...
- **Periodic missed-firing mitigation**: register a periodic job with `RunOnStart: true` + unique-per-day key; insert a row simulating yesterday's run; restart the client; assert exactly one new firing happens for today (the unique key prevents duplicates) — proves the leader-gap workaround works.
