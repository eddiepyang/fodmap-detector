...
type ServiceExtractor struct {
	baseURL    string
	pageClient *http.Client // per-page request timeout
	pdfClient  *http.Client // overall PDF deadline + structure calls
...
func (s *ServiceExtractor) ExtractPDF(ctx context.Context, pdfBytes []byte) (MenuExtractionResult, error) {
	// Overall PDF deadline via pdfClient, but also honor caller ctx.
...
func (s *ServiceExtractor) inspectDocument(ctx context.Context, pdfBytes []byte) (documentInspectResult, error) {
	var res documentInspectResult
	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		s.baseURL+"/v1/documents:inspect", bytes.NewReader(pdfBytes))
	if err != nil {
...
func (s *ServiceExtractor) extractPage(ctx context.Context, pdfBytes []byte, page int) (extractPageResult, error) {
	var res extractPageResult
	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		s.baseURL+"/v1/documents/menu/pages/"+itoa(page)+":extract",
...
// IsEmptyMenuError reports whether err is a service validation error indicating
// an empty menu (the Python schema requires sections min_length=1, so an empty
// menu surfaces as 422/400 rather than an empty result). The detector treats
// this as "no items found" rather than a hard failure.
func IsEmptyMenuError(err error) bool {
	var se *serviceError
	if !errors.As(err, &se) {
		return false
	}
	...
	// Validation-error bodies mention "sections" min_length, or the
	// structuring backend returned an empty/invalid document. Heuristic match.
	msg := strings.ToLower(se.message)
	return strings.Contains(msg, "sections") || strings.Contains(msg, "min_length") ||
		strings.Contains(msg, "empty") || strings.Contains(msg, "no menu")
}
