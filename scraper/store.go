...
// Store persists scraped restaurant and menu data to SQLite.
// It doubles as the OSM discovery cache so we don't re-query Overpass on every run.
type Store struct {
	db *sql.DB
}

// NewStore opens (or creates) the scraper SQLite database at the given path.
...
func (s *Store) UpsertRestaurants(ctx context.Context, restaurants []OSMRestaurant) error {
...
func (s *Store) SearchRestaurants(ctx context.Context, query string) ([]OSMRestaurant, error) {
...
func (s *Store) SaveMenu(ctx context.Context, osmID int64, result *ScrapeResult) (string, error) {
...
func (s *Store) SaveAnalysis(ctx context.Context, menuID string, analysis *MenuAnalysis) error {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
